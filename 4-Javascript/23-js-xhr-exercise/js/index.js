...
xhr.onload = function(){
    if (xhr.status == 200){
        ...
        console.log(convert.hits)
        

        let counter = 0
        setInterval(()=>{
            if(counter < convert.hits.length){
            document.write(`<a href="${convert.hits[counter].pageURL}" target="_blank"><img src="${convert.hits[counter].largeImageURL}" width=800><br></a>`)  
